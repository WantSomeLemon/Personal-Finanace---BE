...
public class GoalsController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<BaseResponse> getGoal(@PathVariable("id") Long id)
    {
        Optional<Goals> goal = goalsService.getGoal(id);
        if(goal.isPresent()){
            return ResponseEntity.ok(new BaseResponse("success", goal.get()));
        }else{
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new BaseResponse("could't find goal"));
        }
    }
